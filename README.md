...
Self-hosted web application (Docker) to control a bidirectional **Alervites/Baofeng AT2** radio from a Linux server, or directly from the browser via local BLE — channels (read/write), device settings, offline messaging (text/image/voice), real-time PTT, position/SOS, authentication.
...
| Local BLE mode: receiving messages | ⚠️ | Previously not implemented at all; now ported to JS, byte-exact-validated — needs a hardware re-test. |
| Local BLE mode: sending voice/image | ⚠️ | Previously text-only; AMR encoder + canvas-based image resize added client-side. |
| Position/SOS | ⚠️ | Piggybacks on the text messaging channel — no structured "Position" type exists in the real protocol. |
| Reconnecting to known devices | ⚠️ | UI bug fixed (mode toggle didn't follow a BLE-local reconnect). |
| Passive "someone is talking" RX indicator | ⚠️ | Fixed once already after a live test; server-mode `/ws/ptt-rx` may need the same fix, untested. |
| Bulk codeplug read/write (all 30 channels at once) | ❌ | The radio simply doesn't respond to this request at all. |
| Old channel-selection command | ❌ | Abandoned — replaced by the fixed quick-select command above. |
| Structured "Position" message type | 📋 | Currently sent as formatted text. |
| Multiple radios at once | 📋 | One connection at a time server-side; local BLE mode allows one radio per browser tab. |
...
<details>
<summary><strong>❌ Details — confirmed not working / abandoned</strong></summary>
