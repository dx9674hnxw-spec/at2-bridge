...
const PttAmrCodec = (() => {
  ...
  class PttAmrCodec {
    constructor() {
      if (typeof AMR === "undefined" || typeof Module === "undefined") {
        throw new Error("amrnb.js must be loaded before ptt-amr-codec.js");
      }
      ...
      this._encInPtr = Module._malloc(FRAME_SAMPLES * 2);
      this._encOutPtr = Module._malloc(32);
      this._decInPtr = Module._malloc(ENCODED_FRAME_BYTES + 1);
      this._decOutPtr = Module._malloc(FRAME_SAMPLES * 2);
    }
    ...
    encode(pcm) {
      ...
      const inView = new Int16Array(Module.HEAPU8.buffer, this._encInPtr, FRAME_SAMPLES);
      inView.set(pcm);
      ...
      const outView = new Uint8Array(Module.HEAPU8.buffer, this._encOutPtr, ENCODED_FRAME_BYTES + 1);
    ...
    decode(frame) {
      ...
      const inView = new Uint8Array(Module.HEAPU8.buffer, this._decInPtr, ENCODED_FRAME_BYTES + 1);
      inView[0] = MR475_TOC;
      inView.set(frame, 1);
      AMR.Decoder_Interface_Decode(this._decState, this._decInPtr, this._decOutPtr, 0);
      const outView = new Int16Array(Module.HEAPU8.buffer, this._decOutPtr, FRAME_SAMPLES);
    ...
    close() {
      ...
      if (this._encInPtr) { Module._free(this._encInPtr); this._encInPtr = null; }
      if (this._encOutPtr) { Module._free(this._encOutPtr); this._encOutPtr = null; }
      if (this._decInPtr) { Module._free(this._decInPtr); this._decInPtr = null; }
      if (this._decOutPtr) { Module._free(this._decOutPtr); this._decOutPtr = null; }
    }
  }
 
  return { PttAmrCodec, FRAME_SAMPLES, FRAME_BYTES_PCM, ENCODED_FRAME_BYTES };
})();